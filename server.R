...
shinyServer(function(input, output, session) {
  ...
  foo <- observeEvent(input$cookie, {
    ...
    output$filesBox <- renderInfoBox({
      files <- sum(ctf_center_study_summary_df$fileId)
      infoBox(
    ...
    output$samplesBox <- renderInfoBox({
      samples <- sum(
        sum(ctf_center_study_summary_df$individualID),
        # sum(ctf_center_study_summary_df$cellLine),
        sum(ctf_center_study_summary_df$specimenID)
      )
      infoBox(
    ...
    output$annotationkey_counts <- plotly::renderPlotly({
      ...
      ctf_summary_df %>%
        ...
        plot_sample_counts_by_annotationkey_2d_ctf(
    ...
    observeEvent(input$center_summary_rows_selected, {
      output$center_name <- renderText({
        ...
        ctf_center_summary_df[[center_row, "name_project"]]
      })
    })
    ...
    observeEvent(input$center_summary_rows_selected, {
      output$center_metadata <- function() {
        center_row <- input$center_summary_rows_selected
        selected_center <- ctf_center_summary_df[[center_row, "projectId"]]
        center_data_df <- ctf_center_summary_df %>%
