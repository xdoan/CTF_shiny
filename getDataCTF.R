...
ctf_summary_df <- get_table_df("syn19040577", cache = TRUE) ## moved to ctf folder
...
ctf_center_study_summary_df <- ctf_summary_df %>% 
...
ctf_center_study_summary_df <- ctf_center_study_summary_df %>%
  group_by(projectId) %>%
  fill(consortium) %>%
  fill(consortium, .direction = "up")

ctf_center_summary_df <- ctf_center_study_summary_df %>% 
  group_by(projectId) %>% 
  summarise(study = n_distinct(study, na.rm = TRUE)) %>% 
  ungroup() %>% 
  left_join(
    ctf_center_study_summary_df %>% 
      select(-study) %>% 
...
ctf_consortium_counts <- ctf_summary_df %>% 
  group_by(consortium) %>% 
  summarize_at(
...
plot_ctf_df <- ctf_center_study_summary_df %>% 
...
plot_sample_counts_by_annotationkey_2d_ctf <- function(
